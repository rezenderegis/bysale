...
public class EnviaClientesTask extends AsyncTask<Integer, Double, String> {

	private Activity context;
	private ProgressDialog progress;

	public EnviaClientesTask(Activity context) {
	...
	protected String doInBackground(Integer... params) {
		...
		/*
		 * // SINCRONIZAÇÃO DE CLIENTES List<Pessoa> pessoas = dao.getLista();
		 * String dadosJson = new PessoaConveter().toJson(pessoas,context);
		 * final String respostaJSON = client.post(dadosJson);
		 * 
		 * //SINCRONIZAÇÃO DE PRODUTOS
		 * 
		 * List<Produto> produto = dao.getListaProduto(); String
		 * dadosProdutoJson = new
		 * PessoaConveter().toJsonProduto(produto,context); WebClient
		 * clientProduto = new WebClient(urlServer); final String
		 * respostaJSONProduto = clientProduto.post(dadosProdutoJson);
		 */
